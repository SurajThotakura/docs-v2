...
**`themeSettings`**
    ...
    **Colors (CSS stylesheet):**
    ...
    > **Note:** The `themeSettings.variables` and `styling` prop use `--auth0-*` prefixed names (e.g., `--auth0-primary`) for color variables. The bare names shown above are only for CSS stylesheet overrides in shadcn/Tailwind projects.
