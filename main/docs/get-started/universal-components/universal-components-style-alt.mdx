...
Import the scoped stylesheet in your application entry point.
...
**Next.js:** Import in your root `layout.tsx` alongside the provider setup. See [Configure Auth0ComponentProvider](/docs/get-started/universal-components/auth0-component-provider) for the full layout example.
...
Auth0 Universal Components work natively with Tailwind v4 (including shadcn). Instead of importing a pre-built stylesheet, your own Tailwind build generates the component CSS.

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
**Requires Tailwind v4.** If you're on Tailwind v3, use the **Without Tailwind** tab — the scoped stylesheet works alongside any Tailwind version.
...
If you're using shadcn, this is already in your CSS. Otherwise, add the following `@theme` block:
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
**Next.js:** Set up your provider with proxy mode as shown in [Configure Auth0ComponentProvider](/docs/get-started/universal-components/auth0-component-provider), then follow the CSS steps above.
...
### Override with your stylesheet

Set CSS custom properties in your stylesheet to override defaults globally.
...
**Border radius and typography** use the same names regardless of your setup:
...
### Override with `themeSettings.variables`
...
Use `common` for radius and typography. Use `light` and `dark` for color and shadow overrides. Color variables always use `--auth0-*` prefixed names.

---

### Override per component with the `styling` prop
